...
//要求：
//Adobe Photoshop CS2或者更高版本

//代码获取：https://github.com/antipalindrome/Photoshop-Export-Layers-to-Files-Fast
...
var ExportLayerTarget = {
    ALL_LAYERS: 1,
    SELECTED_LAYERS: 2, //导出选择，其余部分保持原样，父组的可见性将被强制设置。
...
var DEFAULT_SETTINGS = {
    ...
    padding: app.stringIDToTypeID("填充"),
...
function exportLayers(exportLayerTarget, progressBarWindow) {
    ...
    if ((layerCount == 1) && (layers[0].layer.isBackgroundLayer || prefs.exportForeground)) {
        //扁平化图像不支持 LayerComps 或可见性切换，因此直接导出。
        ...
        //导出所有图层时，关闭所有图层，即使是看似不可见的图层。
        //当切换可见性时，父组变为可见，之前不可见的子组可能会意外变为可见。
        for (var i = 0; i < count; ++i) {
...
function createProgressBar() {
    ...
    try {
        ...
        alert("Progress bar resource is corrupt! Please, redownload the script with all files.", "错误", true);
        return false;
    }
...
function showDialog() {
    ...
    try {
        ...
        alert("Error opening dialog! Please, file an issue and try an older version.", "错误", true);
        return false;
    }
	...
	if(prefs.destinationAsDoc) 
		fields.txtDestination.text = Folder(app.activeDocument.path).fsName;
    else        
    ...
    fields.cbDestinationAsDoc.onClick= function() {
        ...
        if(this.value) 
        {
			prefs.destination = Folder(app.activeDocument.path).fsName;
			fields.txtDestination.text = Folder(app.activeDocument.path).fsName;
        }
        else
        {
    ...
    fields.btnBrowse.onClick = function() {
        var newFilePath = Folder.selectDialog("选择文件夹作为保存位置", prefs.destination);
        if (newFilePath) {
    ...
    dialog.defaultElement = fields.btnRun;
...
function bootstrap() {
    ...
    try {
        ...
        if (env.version < 9) {
            alert("Photoshop versions before CS2 are not supported!", "错误", true);
            return "取消";
        }
        ...
        //     app.activeDocument.suspendHistory('将图层导出为文件', 'main()');
        // } else {
        //     main();
        // }
...
function collectLayersAM(progressBarWindow) {
    ...
    if (layerCount == 0) {
        ...
        if (progressBarWindow) {
            ...
            showProgressBar(progressBarWindow, "计算导出图层...", (layerCount + FEW_LAYERS) / FEW_LAYERS);
        }
...
function countLayersAM(progressBarWindow) {
    ...
    if (layerCount == 0) {
        ...
        if (progressBarWindow) {
            //The layer count is actually + 1 if there's a background present, but it should be no biggie.
            showProgressBar(progressBarWindow, "正在计算图层...", (layerCount + FEW_LAYERS) / FEW_LAYERS);
        }
...
function exportPng8AM(fileName, options) {
    ...
    var id13 = app.charIDToTypeID("Intr"); //交错
    ...
    switch (options.colorReduction) {
        ...
        case ColorReductionType.BLACKWHITE:
            desc4.putString(id361, "黑 & 白");
            break;

        case ColorReductionType.GRAYSCALE:
            desc4.putString(id361, "灰度");
            break;
    ...
    var id21 = app.charIDToTypeID("Dthr"); //仿色算法
    ...
    var id30 = app.charIDToTypeID("TDth"); //透明度仿色算法
    var id31 = app.charIDToTypeID("IRDt");
    var id32;
    switch (options.transparencyDither) {

        case Dither.NONE:
            id32 = app.charIDToTypeID("无");
            break;
...
function loadResource(file) {
    var rsrcString;
    if (!file.exists) {
        alert("Resource file '" + file.name + "' for the export dialog is missing! Please, download the rest of the files that come with this script.", "错误", true);
        return false;
    }
    try {
        file.open("r");
        ...
        alert("Failed to read the resource file '" + file.name + "'!\n\nReason: " + error + "\n\nPlease, check it's available for reading and redownload it in case it became corrupted.", "错误", true);
        return false;
    }
...
function makeMainDialog() {    
    ...
    cbDestinationAsDoc.helpTip = "保存到PSD文件的旁边"; 
    cbDestinationAsDoc.preferredSize.width = 200; 
    cbDestinationAsDoc.text = "导出到PSD文档旁边"; 
    ...
    //该组用于处理忽略某些图层的前缀设置
    //===============
    var grpIgnorePrefix = grpIgnore.add("group", undefined, {name: "该组用于处理忽略某些图层的前缀设置"}); 
    ...
    cbIgnorePrefix.text = "忽略该符号为开头的图层"; 
    ...
    var ddNameAs_array = ["图层名(忽略扩展)","图层名(保留扩展)","图层和图层编组名","从上到下添加编号","从下到上添加编号"]; 
    ...
    cbDelimiter.helpTip = "比如替换图层名称中的空白符号"; 
    cbDelimiter.text = "自定义图层名称的分隔符"; 
    ...
    lblLetterCasing.text = "字母"; 

    var ddLetterCasing_array = ["不变","全部小写","全部大写"]; 
    ...
    docNamePrefix.helpTip = "保留PSD文件名作为图层名前缀，示例：PSD文件名称_图层名称"; 
    docNamePrefix.text = "PSD文件名作为前缀"; 
    ...
    cbOverwriteFiles.text = "覆盖相同名称文件"; 
    ...
    cbGroupsAsFolders.text = "根据编组层级建立文件夹目录"; 
    ...
    cbTopGroupsAsFolders.text = "一级组名称作为文件夹"; 
    ...
    cbTopGroupsAsLayers.text = "合并组为图层导出"; 
    ...
    cbForeground.text = "最高图层作为其他图层的前景"; 
    ...
    var ddTrim_array = ["每一层","每一层(裁切)","结合"]; 
    ...
    //PNLEXPORTAS
    //===========
    var pnlExportAs = dialog.add("panel", undefined, undefined, {name: "pnlExportAs", borderStyle: "无"}); 
    pnlExportAs.text = "文件类型"; 
    ...
    lblPng8ColorReduction.text = "减低颜色深度算法"; 

    var ddPng8ColorReduction_array = ["可感知","可选择","随样性","受限(web)","-","黑 & 白","灰度","Mac OS","Windows"]; 
    ...
    lblPng8TransparencyDither.text = "透明度仿色算法"; 
    ...
    lblPdfDestinationProfile.text = "输出"; 
    ...
    var ddPdfDownSample_array = ["不进行降采样","平均降采样","子采样（Subsampling）","降采样（downsampling）"]; 
    ...
    lblPdfDownSampleSizeLimit.text = "超过指定分辨率后的处理策略"; 
    ...
    lblMetadata.text = "所有图层数量：{0} 个，可见图层数量：{1} 个     "; 
    ...
    lblContact.add("statictext", undefined, "想要关注版本更新，或者提交反馈，可访问：", {name: "lblContact"}); 
